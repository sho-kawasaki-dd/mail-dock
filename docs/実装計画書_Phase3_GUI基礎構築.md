...
| F-3 | ウィザードのルート選択で、ドライブ種別・空き容量を確認し、暗号化状態が確認できない場合に警告を表示すること。`.maildock_root` の初期化を行うこと。**Phase 3.5 で、ストレージ適合性セルフテストと暗号化状態の3値ユーザー申告（`encrypted` / `unencrypted` / `unknown`）を追加する** | 5.3 / 3.6 / Phase 3.5 |
...
- [x] ページ1: ストレージルート選択
  ...
  - [x] 暗号化状態を確認できない場合に警告を表示する（BitLocker To Go 前提。開発計画書 5.3）。**この要件は Phase 3.5 で、自動検出ではなく `encrypted` / `unencrypted` / `unknown` の3値ユーザー申告へ置き換え、ストレージ適合性セルフテストを追加した**
