...
4. **ネットワークドライブの制限:** SQLiteのWALモードはSMB共有上で正しく動作しない。ルート選択時にドライブ種別とストレージ適合性セルフテストの結果を確認し、`journal_mode` は両者の安全側の組み合わせで決定する。ネットワークドライブ（`DriveKind.NETWORK`）では、セルフテストでWALが利用可能に見えても常に `journal_mode=DELETE` とし、警告を表示する。ローカルドライブでは、セルフテストのWAL測定が成功した場合だけ `WAL` を使用し、失敗時は `DELETE` にフォールバックする。
...
11. **ストレージ適合性セルフテスト:** 製品名ではなく、ストレージルート上で実際に成立する能力を測定する互換性プローブを、ルート初期化時および必要な再検査時に実行する。排他ロック競合、既存ファイルへの `os.replace` 上書き、SQLiteのWAL、ファイルおよび（POSIXでは）ディレクトリの `fsync`、大文字小文字の区別、想定最大長のパス作成を測定し、結果を `OK` / `DEGRADED` / `UNSUPPORTED` に集約する。排他ロックまたは上書き配置が成立しない場合は `UNSUPPORTED`、WALまたは `fsync` が成立しない場合は `DEGRADED` とする。大文字小文字の区別と長いパスの結果は記録するが、単独では判定を下げない。`UNSUPPORTED` の新規ルートは選択を拒否し、既存の運用ルートでは測定結果を保存したうえで、GUIの確認を経た場合だけ続行を許可する。
    * **製品名で判定しない理由:** Cryptomator等の仮想ファイルシステムや、将来登場する同種のツールを製品名から確実に検出する手段はなく、製品名の拒否リストでは未知の非互換性を取りこぼすためである。ただし、単発のI/O成功で原子性・永続性・WALの安全性を完全に証明できるわけではなく、セルフテストは既知の非互換性を検出するプローブである。
    * **非破壊性と結果の保存:** テストは本番の `.lock` / `metadata.db` / `eml/` / `manifests/` に触れず、ストレージルート配下の `tmp/` に作る一時ファイルだけで完結し、終了時に残骸を残さない。測定結果はストレージルートではなく内蔵ディスク側の `config.json` に `root_uuid` と紐付けて保存する。測定時の正規化パスとストレージ指紋が現在値と一致するときだけキャッシュを再利用する。これにより、ドライブ全体のコピーで同じUUIDが引き継がれても、別媒体の古い適合性判定を持ち回らない。
...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    provider_type TEXT NOT NULL,      -- 'imap'（通常IMAP・Gmail・Microsoft 365） / 'pst_import'
    display_name  TEXT,
...
**IMAPフラグの扱い（定期的に取り直すスナップショット方式）**

* サーバー側の `\Seen` / `\Flagged` / `\Answered` 等を同期時および定期リフレッシュ時に一方向で取り直して保存し、**閲覧時の表示にのみ使用**する。
* **双方向同期は行わない**。ローカルでの操作をサーバーへ反映する機能はスコープ外とする（バックアップ用途では不要であり、複雑度に見合わない）。
* **ローカル独自の既読管理も行わない**。本アプリは閲覧専用アーカイブとして位置づける。
* `flags_seen_at` を併記することで、UI上で「この情報を最後に確認した日時」を示せるようにする。
...
| `imap_flags` に `\Seen` 無し | 未読アイコン（**最終確認日時つきのスナップショットである旨をツールチップで明示**） |
...
* **バックアップの多重化:** バックアップ方法は暗号化方式によって分ける。デバイス全体または通常のボリュームを暗号化している場合は、マウントしたストレージルートを別の暗号化済み媒体へ通常のファイルコピー（robocopy / FreeFileSync 等）で複製できる。VeraCryptのファイルコンテナを使用している場合は、アプリとVeraCryptボリュームを停止して**アンマウントした後**、コンテナファイル自体を丸ごとコピーする。コンテナ運用では差分バックアップを行わず、マウント中のコンテナファイルをコピーしてはならない。コピー先の暗号化状態が保管元より弱い場合は警告し、既定では実行しない。いずれの方式でも、この点をREADMEおよびアプリ内ヘルプに明記し、**3-2-1ルール（3コピー・2媒体・1オフサイト）**を推奨する。

> `metadata.db.bak` をストレージルート内だけに置くと、ドライブ障害で同時に失われる。内蔵ディスク（`{config_dir}`）へメタデータのみのバックアップを1世代置く案は有効だが、**DBには件名・差出人が含まれる**。そのため複製先の暗号化状態が保管元より弱い場合は情報漏洩リスクを警告し、内蔵ディスクへの複製は既定でOFFとする。ユーザーが複製先の保護状態を確認したうえで明示的に有効化した場合だけ実行する。これは特定の暗号化製品や C: ドライブに依存しない。

> VeraCryptのファイルコンテナをバックアップする場合は、コンテナ運用の必須手順として**ボリュームヘッダのバックアップ**を取得する。ヘッダを失うとコンテナ内のデータへアクセスできなくなるため、コンテナ本体とは別の安全な場所へ保管し、復元手順も確認しておく。
...
* **自動ロック／自動アンマウントの無効化:** Vault製品のアイドル自動ロックや、VeraCryptのスクリーンセーバー起動時・一定時間経過時などの自動アンマウントも、物理的な抜去と同じ切断イベントを発生させる。数時間規模になり得る初回同期中に発火すると、書き込み途中の停止・復帰処理が必要になるため、アプリが使用する保管先ではこれらを無効化することを推奨する。自動アンマウントを無効化できない環境では、長時間同期を実行せず、同期完了後に明示的にアンマウントする。
...
| ストレージ適合性セルフテスト | Qt に依存しない通常の単体テストとして実行する。排他ロック、`os.replace` 上書き、WAL、fsync、大文字小文字の区別、長パスの各能力について失敗を注入し、`OK` / `DEGRADED` / `UNSUPPORTED` の集約と `journal_mode` の安全側決定を検証する |
...
| 保管先の暗号化状態（申告） | **`unknown`** | `encrypted` / `unencrypted` / `unknown` の3値。自動検出は行わず、UIに常時表示 |
| 資格情報の保存方式 | **`keyring`** | 許可外バックエンドでは保存せず、`session_only` へフォールバック |
| ストレージ適合性の確認結果 | **ルート確定時に自動測定** | `config.json` に `root_uuid` と紐付けて保存し、測定時パスとストレージ指紋が一致するときだけ再利用 |
| 瞬断時のリプローブ回数 | **3回（500ms間隔）** | 超過したら `DETACHED` へ遷移 |
| リムーバブル時の `synchronous` | **`NORMAL`** | Phase 4 の実測結果により「コミット時のみ `FULL`」へ変更可（3.6参照）。**実績:** Phase 4 では実機での実測（VHDX detach・フルスケール同期）を延期したため実測できず、既定 `NORMAL` を維持した。切替口（コミット時のみ `FULL`）は設計上残してあり、実測後に設定1箇所の変更で切り替えられる |
...
| **Phase 1: 抽象化層 & IMAPコア** | 1〜2週間 | `BaseMailFetcher` と例外階層の定義、`GenericImapFetcher` 実装、EML解析（文字コード・RFC2231・スレッドヘッダ）、原子的なEML保存とDB登録。**小規模フォルダでのお名前.com実機検証をここで実施**（フォルダ区切り文字、modified UTF-7、同時接続数制限、タイムアウト挙動の確認） |
...
| SSD紛失・盗難による情報漏洩 | 低 | **致命的** | 暗号化状態のユーザー申告 | ブロックレベル暗号化を推奨し、暗号化なしも明示的な申告で許可する。`encrypted` / `unencrypted` / `unknown` の状態をUIに常時表示し、`unencrypted` / `unknown` では初回同期前に一度だけ強い確認を行う。資格情報はPC側の許可済み `keyring` または `session_only` に限定し、ストレージルートへ保存しない |
...
| **ファイル単位暗号化ツール上での運用による原子性・ロックの喪失** | 中 | **高** | ストレージ適合性セルフテスト | 製品名ではなく排他ロック、`os.replace` 上書き、WAL、fsync等の能力を測定する。排他ロックまたは上書き配置が成立しない場合は `UNSUPPORTED` として新規ルートの選択を拒否し、既存ルートで続行を承認した場合も常時警告を表示する。WALまたはfsyncの失敗は `DEGRADED` とする |
...
| IMAPフラグ | サーバーから定期的に取り直すスナップショットを保存し表示のみ。双方向同期は行わない |
...
| ストレージ適合性の判定方法 | 製品名ではなく、排他ロック、`os.replace` 上書き、WAL、fsync等の能力を `tmp/` 配下で測定する。将来の未知のツールにも対応でき、既知の非互換性を `OK` / `DEGRADED` / `UNSUPPORTED` として扱えるため |
