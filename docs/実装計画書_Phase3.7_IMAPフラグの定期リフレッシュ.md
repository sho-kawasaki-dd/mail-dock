...
| D-8 | 永続化方法 | `BaseMessageRepository` に `imap_flags` / `flags_seen_at` を更新する専用メソッドを追加する。フラグ変化があったメッセージは個別値更新、変化がなかったメッセージはN+1問題を回避するためUIDリストによる一括 `flags_seen_at` 更新（バッチ実行）を行う |
...
| D-10 | `HIGHESTMODSEQ` の保持 | `folders` テーブルに `highest_modseq INTEGER` 列を追加し、フォルダ単位で前回確認済みのMODSEQを保持する。`CONDSTORE` 非対応サーバーでは常に `NULL` のままとする。`UIDVALIDITY` 変更時は `NULL` にリセットする |
| D-11 | バッチ制御 | 既存の `begin_batch` / `commit_batch` / `checkpoint` をそのまま再利用する。フラグリフレッシュのDB更新と `highest_modseq` の進捗は同一トランザクション内でコミットする |
...
| F-6 | `BaseMessageRepository` に、`internal_date >= since_internal_date` かつ `(flags_seen_at < max_seen_at OR flags_seen_at IS NULL)` なメッセージのUID一覧を返す候補取得メソッドを追加する | D-4, D-5 |
| F-7 | フラグリフレッシュ時のDB更新: 変化があったメッセージは `update_flags` でフラグと `flags_seen_at` を更新し、変化がなかったメッセージは `touch_flags_seen_at` による一括更新で `flags_seen_at` を最新化して次回のTTLを効かせる（N+1クエリを発生させない） | D-5, D-8 |
...
* 既存の `BaseMessageRepository` に「目的を超えたメソッドを足さない」という制約（設計不変条件）を踏まえ、追加するメソッドはフラグ更新・確認時刻一括更新・リフレッシュ対象UID取得に限定する。
...
- [ ] `select_folder()` で `CONDSTORE` サポート時に `(CONDSTORE)` パラメータまたは有効化を行い、`HIGHESTMODSEQ` 応答を保持するアクセサを実装する
...
- [ ] `BaseMessageRepository` に `list_flag_refresh_candidates(account_id, folder_id, uidvalidity, since_internal_date, max_seen_at) -> Sequence[int]` を追加する
...
- [ ] `SqliteMessageRepository.update_sync_cursors` に `highest_modseq: int | None = None` 引数を追加し、`initialize_sync_cursors` では `highest_modseq = NULL` にリセットするよう拡張する
...
- [ ] `CONDSTORE` 対応時の経路を実装:
  - `folders.highest_modseq` を取得し、`iter_flags_since` で差分を取得
  - 差分メッセージに対して `update_flags` を実行
  - 正常完了時にサーバーの最新 `highest_modseq` を `update_sync_cursors` でバッチコミット
- [ ] 非 `CONDSTORE` 時のフォールバック経路を実装:
  - `list_flag_refresh_candidates` で対象UIDを取得
  - `iter_flags` で取得したリモートフラグとローカルフラグを比較
  - 変化があったメッセージは `update_flags`、変化がなかったメッセージは `touch_flags_seen_at`（N+1防止）で一括更新
- [ ] フラグリフレッシュ中の `FetchError` は警告ログに残し、フォルダ・アカウント全体の同期を失敗させない（`highest_modseq` は進めない）
- [ ] `CancelToken` によるキャンセルに対応し、キャンセル時はその時点までのバッチのみ安全にコミットする
...
- [ ] `message_table_model.py` および詳細ビューで `flags_seen_at`（UTC ISO8601）をローカル時刻表記に変換してツールチップを構築する処理を実装する

### **Group F: テスト**

- [ ] `tests/unit/test_sync_mail.py` にフラグリフレッシュの単体テストを追加する（対象範囲・TTL間引き・CONDSTORE経路・フォールバック経路・変化なし時の一括touch・キャンセル時挙動を検証）
- [ ] `tests/unit/test_message_repository.py` に `list_flag_refresh_candidates`, `update_flags`, `touch_flags_seen_at`, `highest_modseq` の単体テストを追加する
- [ ] `tests/unit/test_fetcher.py` / `test_imap_common.py` に単一 bytes の FLAGS レスポンスパースおよび `iter_flags` / `iter_flags_since` のテストを追加する
- [ ] `tests/integration/` に結合テストを追加し、サーバー側のフラグ変更が次回同期で反映されることを検証する
