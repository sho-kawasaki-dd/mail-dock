...
| D-2 | `001_init.sql` の範囲 | **開発計画書3章の全テーブル（PST関連を除く）を一括作成**。Phase 1で `002_sync_cursor.sql`、Phase 4.5で `006_pst_import.sql` を追加する（後続フェーズで確定した採番） |
...
- [x] V-2. `uv run pytest -m "not docker"` が Windows で全緑になる（基盤モジュールのカバレッジ80%目安）
