...
2. **「安全でない保管先」を製品名ではなく能力で判定すること** — Cryptomator 等のファイル単位暗号化ツール（仮想ファイルシステム）上では、設計不変条件2が依存する `os.replace` の原子性・排他ロック・fsync の保証が実装依存になる。これを**ストレージ適合性セルフテスト**で直接測定し、要件を満たさない保管先を検出する。
...
| D-3 | 安全でない保管先の判定方法 | **製品名ではなく能力で判定する**。Cryptomator 等を確実に検出する手段は存在せず、製品名ベースの判定は必ず漏れる。ストレージルート配下で実際に「排他ロックが取れるか」「`os.replace` で上書き配置できるか」等を測定する**ストレージ適合性セルフテスト**を新設する |
...
| D-5 | セルフテスト失敗時の扱い | 致命的失敗（排他ロック不可・`os.replace` 不可）のとき、**ウィザードでの新規ルート選択は拒否**して次へ進ませない。一方、**既に運用中の既存ルート**（アプリ更新後に初めて検査して失敗した場合）は、**1回だけ強い確認を出して続行を許可**し、承認（ack）を記録したうえで以後ステータスバーに常時警告を表示する。既存ユーザーが突然データへアクセスできなくなる事態を避けるため |
| D-6 | 検査結果・申告の保存場所 | **`config.json`（内蔵ディスク側）に `root_uuid` 紐付けで保存する**。ストレージルート配下に置くと、ドライブ丸ごとのコピー（robocopy 等）で作った複製が同じ `root_uuid` と検査結果を引き継ぎ、別の物理媒体に対する誤った判定を持ち回ることになるため |
| D-7 | `journal_mode` の決定 | **セルフテスト結果を正とする**。`PRAGMA journal_mode=wal` の戻り値を直接検査するため、`DriveKind` による推測より正確である。`connect()` / `ConnectionManager` の `network_drive: bool` 引数を `journal_mode: str` へ置換する。`DriveKind` は削除せず、ウィザードのドライブ種別表示と警告ログ用に残す（`VIRTUAL` は追加しない） |
...
| D-11 | 設定スキーマ | **`schema_version` を 1 → 2 へ上げる**。検査結果・承認・暗号化申告・初回同期確認を `storage_profiles`（キー = `root_uuid`）の**1フィールドへ集約**する。ルート候補ごとに独立して保持でき、`root_uuid` が変われば自動的に別エントリになる |
...
| D-13 | 資格情報の代替保存方式 | **`session_only` モードを新設する**。プロセス内のメモリにのみ保持し、ファイル・DB・ログへ一切書かない。許可外バックエンド検出時の自動フォールバック先であると同時に、共用PCで使いたいユーザーが明示的に選べる選択肢でもある |
...
| D-17 | セルフテストの再実行契機 | **`config.storage_profiles[root_uuid]` にキャッシュがある場合は再実行しない**（起動時間 3秒以内の維持のため）。再実行するのは ①該当 `root_uuid` の記録が無いとき ②ウィザードでルートを選択したとき ③設定ダイアログの「再検査」を押したとき、の3つに限る |
...
| F-3 | ストレージルートの初期化時にストレージ適合性セルフテストが実行され、排他ロック・`os.replace` 上書き・WAL・fsync・大文字小文字の区別・長パスの各項目が測定されること | D-3 |
...
| F-7 | 既存の運用中ルートが `UNSUPPORTED` と判定された場合、GUI で1回だけ強い確認を出して続行を許可し、承認を記録すること。承認後はステータスバーに警告を常時表示すること | D-5 / D-18 |
| F-8 | CLI では `UNSUPPORTED` かつ承認未記録のとき `StorageUnsupportedError` で必ず失敗すること。CLI から承認を発行する手段が存在しないこと | D-10 |
| F-9 | SQLite の `journal_mode` がセルフテスト結果から決定され、WAL が使えない保管先では `DELETE` へフォールバックすること。`journal_mode` の値が許可リストで検証されてから PRAGMA へ渡されること | D-7 |
| F-10 | ウィザードで暗号化状態を `encrypted` / `unencrypted` / `unknown` の3値から申告でき、`unencrypted` を選んだ場合のみ確認チェックボックスが必須になること。自動検出を行わないこと | D-4 |
| F-11 | セルフテスト結果・承認・暗号化申告・初回同期確認が `config.json` に `root_uuid` 紐付けで保存され、ストレージルート配下には書き込まれないこと | D-6 / D-11 |
...
| F-18 | `session_only` モードの資格情報ストアが、ファイル・DB・ログへ一切書き込まないこと | D-13 |
...
- [ ] 「BitLocker To Go 等で暗号化することを必須の前提条件とする」を削除し、**3層モデルの表**へ差し替える（D-1）
- [ ] 推奨層に OS 別の手段を列記する（Windows Pro: BitLocker To Go / Windows Home: VeraCrypt / macOS: APFS暗号化・VeraCrypt / Linux: LUKS・VeraCrypt）
- [ ] Windows Home でも**BitLockerで暗号化済みのドライブを解錠して読み書きすることは可能**であり、できないのは作成のみである旨を注記する
- [ ] VeraCrypt について、**デバイス全体の暗号化を第一推奨**、ファイルコンテナを「ドライブを他用途と共用する場合の選択肢」と位置づけ、4条件（固定サイズ必須 / クラウド同期フォルダ・ネットワーク共有に置かない / 自動アンマウント無効化 / ヘッダバックアップ）を明記する（D-19）
- [ ] 非推奨層（ファイル単位暗号化・仮想ファイルシステム）について、`os.replace` の原子性・排他ロック・fsync が実装依存になり**設計不変条件2が保証できなくなる**ことを理由として明記する（D-20）
- [ ] 自己責任層について、明示的な申告を記録して続行を許可する旨と、UI に常時表示する旨を明記する
- [ ] **暗号化状態の自動検出は行わず、ユーザー申告として記録する**（技術的検証を装わない）ことを明記する（D-4）
- [ ] **アプリ層暗号化（7z / AES-ZIP / SQLCipher）を採用しない理由を3点とも記録する**（D-2）
- [ ] 既存の「アプリ内でのDB暗号化（SQLCipher等）は採用しない」の記述を、新しい理由付けと整合するよう更新する
...
- [ ] 既存の項目4（ネットワークドライブの制限）を更新し、`journal_mode` の決定が `DriveKind` からセルフテスト結果へ移ったこと、`DriveKind` は警告表示用に残ることを明記する（D-7）
- [ ] 検査結果を**内蔵ディスク側の `config.json` へ `root_uuid` 紐付けで保存する**理由（ドライブ丸ごとコピーが誤った判定を引き継ぐため）を明記する（D-6）
...
- [ ] 既存の `StorageError` 配下の並びに合わせ、docstring で「原子性・排他ロックの前提が成立しない保管先」であることを1行で示す
...
- [ ] `@dataclass(frozen=True) class StorageCapabilities` を定義する
  - [ ] フィールド: `exclusive_lock: bool` / `atomic_replace: bool` / `wal_supported: bool` / `fsync_supported: bool` / `case_sensitive: bool` / `long_path_ok: bool` / `checked_at: str`（UTC ISO8601）
...
- [ ] `probe_capabilities(root: Path) -> StorageCapabilities` を実装する
  ...
  - [ ] **排他ロック**: `tmp/.captest-{uuid}.lock` に対し Windows は `msvcrt.locking(LK_NBLCK, 1)`、POSIX は `fcntl.flock(LOCK_EX | LOCK_NB)` を取得・解放する。**本番の `.lock` には触れない**
  ...
  - [ ] **長パス**: `eml/{account_id}/{YYYY}/{MM}/{hash32}.eml` の想定最大長に相当するパスでファイルを作成できるか測定する
...
- [ ] `capability_level(capabilities: StorageCapabilities) -> CapabilityLevel` を実装する
  - [ ] `exclusive_lock` または `atomic_replace` が `False` → `UNSUPPORTED`
  - [ ] `wal_supported` または `fsync_supported` が `False` → `DEGRADED`
  - [ ] それ以外 → `OK`
- [ ] `journal_mode_for(capabilities: StorageCapabilities) -> str` を実装する（`"WAL"` / `"DELETE"`）
- [ ] モジュール docstring に「**製品名ではなく能力を測る。本アプリの原子性・排他ロックの前提が実際に成立するかを検証する唯一の関門であり、`tmp/` 配下以外に一切書き込まない**」と明記する
...
- [ ] `AppConfig` に `storage_profiles: Mapping[str, JSONValue]` を追加する
  - [ ] キーは `root_uuid`、値は `{capabilities, capability_level, capability_ack_at, encryption, encryption_declared_at, first_sync_confirmed_at}`
...
- [ ] `journal_mode` の決定責務が呼び出し元（セルフテスト結果）にある旨をコメントで1行残す
...
- [ ] `ensure_layout(root)` / `cleanup_tmp(root)` の直後に、`config.storage_profiles[root_uuid]` へ有効なキャッシュが無い場合のみ `probe_capabilities(root)` を実行する（D-17）
- [ ] `capability_level` が `UNSUPPORTED` かつ `capability_ack_at` が未記録の場合、`StorageUnsupportedError` を送出する（D-10）
- [ ] `journal_mode_for(capabilities)` の結果を `ConnectionManager` へ渡す
...
- [ ] `_save_settings` で `storage_profiles` を更新し、`storage_root_candidates` に対応しない `root_uuid` のエントリを掃除する
- [ ] `config.credential_storage` と `detect_backend()` から資格情報ストアを選択する。`keyring` 指定でバックエンドが許可外なら `SessionCredentialStore` へフォールバックし、警告をログへ出す（D-12 / D-13）
...
- [ ] 既存ルートが `UNSUPPORTED` かつ未承認の場合、`StorageUnsupportedError` を捕捉して**続行確認ダイアログ**を出し、承認されたら `capability_ack_at` を記録して `StorageSession` を再開する（D-5）
...
- [ ] `_validate_root` 内で `probe_capabilities` を実行し、新設の `_capability_label` へ結果を表示する
...
- [ ] 同期開始経路で、当該 `root_uuid` に `first_sync_confirmed_at` が無く暗号化申告が `unencrypted` / `unknown` の場合、**1回だけ**モーダル確認を表示する（D-18）
- [ ] 確認後に `first_sync_confirmed_at` を記録し、以後は確認を出さない
- [ ] 資格情報が `session_only` で動作している場合、その旨をステータスバーまたは通知で1回示す

#### **E-4. `presentation/views/settings_dialog.py` — 設定項目の追加（*E-1 に依存*）**
...
- [ ] `UNSUPPORTED` かつ未承認で `StorageUnsupportedError` が送出されることを検証する
- [ ] 承認済みなら続行できることを検証する
- [ ] キャッシュがある場合にセルフテストを再実行しないことを検証する（D-17）
...
- [ ] `tests/gui/test_setup_wizard.py`: `UNSUPPORTED` で次へ進めないこと、`DEGRADED` では警告付きで進めること、暗号化申告が `config` へ保存されること、`未暗号化` 選択時に確認チェックが必須になること
- [ ] `tests/gui/test_main_window.py`: ステータスバーへ暗号化状態と適合性が表示されること、初回同期前の確認が1回だけ出て記録されること
...
| `src/mail_dock/presentation/views/settings_dialog.py` | 再申告・資格情報の保存方式・再検査 | E-4 |
...
- [ ] V-6. 設定の後方互換: schema v1 の既存 `config.json` を持つ環境で起動し、v2 へ無損失アップグレードされ、未知キーが保持される（N-4）
- [ ] V-7. 通常のローカルドライブでウィザードを一周し、セルフテストが `OK`、`journal_mode=WAL` が適用される。セルフテストの所要時間が1秒以内である（N-1）
- [ ] V-8. 2回目以降の起動でセルフテストが再実行されず、起動時間が3秒以内を維持する（N-2 / D-17）
- [ ] V-9. ネットワークドライブ（SMB共有）をルートに指定すると `DEGRADED` と判定され、`journal_mode=DELETE` へフォールバックする
...
- [ ] V-13. CLI の安全性: `UNSUPPORTED` かつ未承認の状態で CLI コマンドを実行すると `StorageUnsupportedError` で失敗し、CLI から承認を発行する手段が存在しない（F-8）
- [ ] V-14. 資格情報: 許可外の keyring バックエンドを模擬した状態で、資格情報が保存されず `session_only` へフォールバックし、警告が表示される。`SessionCredentialStore` がファイルを一切作成しない（F-16 〜 F-18）
...
- [ ] V-17. 実測（可能なら）: Cryptomator Vault をストレージルートに指定し、`UNSUPPORTED` または `DEGRADED` として検出されるか実測する。検出できない場合はその結果を本書「7.」へ記録し、開発計画書 5.3 の非推奨理由へ反映する
...
* **V-17 / V-18 の実測結果を必ず記録すること。** 特に Cryptomator 上でセルフテストが `OK` を返してしまった場合は、判定項目が不足していることを意味する。追加すべき測定項目（例: `os.replace` 後の即時 `stat` 整合性、ロックの実効性を2プロセスで確認する等）を検討し、開発計画書 2.4 へ反映する。
...
* **`storage_profiles` の肥大化。** ルートを頻繁に付け替えるユーザーではエントリが増える。`_save_settings` の掃除ロジックが `storage_root_candidates` に依存しているため、Phase 4 で複数ルート候補を扱う場合は掃除条件を見直すこと。
