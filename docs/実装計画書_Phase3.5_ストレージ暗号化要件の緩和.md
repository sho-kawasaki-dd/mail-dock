...
**「ストレージルートは特定の製品で暗号化されていること」という従来の必須要件を撤廃し、Windows 11 Home・macOS・Linux のユーザーが本アプリを安全に利用できるようにする。**

現行の 5.3 は特定の暗号化方式を必須の前提条件としていたため、事実上 Windows 11 Pro 以上でしか正規の運用ができない状態だった。これはアプリのアクセシビリティを不必要に狭めていた。
...
| # | 要件 | 根拠 |
| :--- | :---- | :---- |
| F-1 | 開発計画書 5.3 の「保存データの暗号化」が3層モデルへ書き換えられ、ブロックレベル暗号化が「必須の前提条件」ではなく「推奨手段のひとつ」として記述されていること | D-1 |
...
- [x] 「特定の暗号化方式を必須の前提条件とする」という旧記述を削除し、**3層モデルの表**へ差し替える（D-1）
...
- [x] `keyring` バックエンドを**許可リスト方式**で検査すること、許可外では保存しないことを明記する（D-12）
- [x] Linux で D-Bus / デスクトップ環境が無い場合に `keyrings.alt` の平文バックエンドへフォールバックし得る危険性を明記する
- [x] `session_only` モード（プロセス内メモリのみ）の存在と、その用途（許可外バックエンドのフォールバック先 / 共用PCでの明示的選択）を追記する（D-13）
- [x] **資格情報をストレージルート配下へ書き込むことを明示的な禁止事項として格上げする**。理由（未暗号化ドライブの紛失がアカウント乗っ取りへ波及しない分離を維持するため）を併記する（D-14）
- [x] 資格情報は PC 側、メールデータはストレージルート側という**保管場所の分離**が、暗号化要件の緩和後もそのまま有効であることを明記する

#### **A-3. 開発計画書 2.4「設計上のポイント」への項目追加**

- [x] 新項目「**ストレージ適合性セルフテスト**」を追加し、測定項目・判定値（`OK` / `DEGRADED` / `UNSUPPORTED`）・失敗時の挙動を記述する（D-3 / D-5）
- [x] 判定を**製品名ではなく能力で行う**理由（確実な検出手段が無く、将来のツールにも対応する必要があるため）を明記する
- [x] `tmp/` 配下のみを使う非破壊テストであることを明記する（D-8）
- [x] 既存の項目4（ネットワークドライブの制限）を更新し、`journal_mode` をセルフテスト結果と `DriveKind` の安全側の組合せで決定すること、ネットワークドライブは常に `DELETE` とすることを明記する（D-7）
- [x] 検査結果を**内蔵ディスク側の `config.json` へ `root_uuid` 紐付けで保存し、測定時パスとストレージ指紋が一致するときだけ再利用する**理由（ドライブ丸ごとコピーが同じUUIDを引き継いでも別媒体の判定を持ち回らないため）を明記する（D-6）

#### **A-4. 開発計画書 5.7 / 5.7.1 の更新**

- [x] 5.7 の「ドライブ丸ごとのコピーが完全なバックアップになる」を、**暗号化方式別に書き分ける**（デバイス暗号化時＝通常のファイルコピーで可 / ファイルコンテナ運用時＝アンマウントしてコンテナファイルを丸ごとコピー、差分バックアップ不可、マウント中のコピー禁止）
- [x] VeraCrypt のボリュームヘッダバックアップを、コンテナ運用時の必須手順として追記する（D-19）
- [x] 5.7 の `metadata.db.bak` に関する但し書きを、BitLocker 固有の条件から「**複製先の暗号化状態が保管元より弱い場合は警告し、既定でOFF**」へ一般化する（D-16）
- [x] 5.7.1 に、**Vault のアイドル自動ロック**と **VeraCrypt の自動アンマウント**（スクリーンセーバー起動時等）を切断イベントの発生源として追記する。数時間規模の初回同期中に発火し得るため、無効化を推奨する旨を併記する

#### **A-5. 開発計画書 5.10 / 5.11 / 7章 / 8章の更新**

- [x] 5.10 テスト方針に「**ストレージ適合性セルフテスト**」の行を追加する（各能力の失敗を注入する単体テスト。Qt非依存で通常CIで実行）
- [x] 5.11 主要設定項目に3行を追加する: 「保管先の暗号化状態（申告）＝ 既定 `unknown`」「資格情報の保存方式 ＝ 既定 `keyring`」「ストレージ適合性の確認結果 ＝ ルート確定時に自動測定」
- [x] 7章リスク管理の「SSD紛失・盗難による情報漏洩」行の対応策を、3層モデルと申告・常時表示の仕組みへ書き換える
- [x] 7章リスク管理に新規行「**ファイル単位暗号化ツール上での運用による原子性・ロックの喪失**」を追加する（検知方法＝セルフテスト、対応策＝`UNSUPPORTED` 判定と続行時の常時警告）
- [x] 8章決定事項ログの「保存データ暗号化」行を3層モデルへ書き換える
- [x] 8章決定事項ログに3行を追加する: 「**アプリ層暗号化の不採用**（理由3点）」「**資格情報の保存方式**（keyring 許可リスト / session_only）」「**ストレージ適合性の判定方法**（製品名ではなく能力測定）」

#### **A-6. 既存実装計画書の該当箇所の修正**

- [x] `実装計画書_Phase0_基盤整備.md` の前提条件「ストレージルートの BitLocker To Go による暗号化（5.3）」を3層モデルの記述へ修正する
- [x] 同 D-9（`metadata.db.bak` の内蔵ディスク複製）の条件記述を D-16 の一般化された条件へ修正する
- [x] 同 Phase 4 への申し送り（`db_backup_to_local_disk` の実処理条件）を同様に修正する
- [x] `実装計画書_Phase3_GUI基礎構築.md` の F-3（ウィザードのルート選択要件）に、セルフテストと暗号化申告が Phase 3.5 で追加された旨の追記を行う
- [x] 同 E-1（ウィザード）の「暗号化状態を確認できない場合に警告を表示する」タスクに、Phase 3.5 で3値申告へ置き換えた旨の追記を行う

#### **A-7. README への暗号化ガイド追加**

- [x] 「保管先の暗号化」セクションを新設し、3層モデルを簡潔に説明する
- [x] OS別の暗号化手順（Windows Pro / Windows Home / macOS / Linux）を記載する
- [x] VeraCrypt の4条件を記載する（D-19）
- [x] 非推奨ツールとその理由を記載する（D-20）
- [x] 自動ロック・自動アンマウントの無効化と、アンマウントしてからシャットダウンする運用を記載する
- [x] 3-2-1ルールのバックアップ推奨（開発計画書 5.7）への導線を張る
...
- [x] 「保管先の暗号化状態」の再申告（3値コンボ）を追加し、変更を `config.storage_profiles` へ保存する
- [x] 「資格情報の保存方式」（`keyring` / `session_only`）を追加する。検出済みバックエンド名を読み取り専用で表示する
- [x] `keyring` を選べない環境（許可外バックエンド）では選択肢を無効化し、理由を表示する
- [x] 「ストレージ適合性」の結果表示と「**再検査**」ボタンを追加する（D-17）
...
- [x] V-16. ドキュメント: 開発計画書 5.3 / 2.4 / 5.7 / 5.7.1 / 5.10 / 5.11 / 7章 / 8章と README が改訂され、特定の暗号化方式を必須の前提条件とする旧記述がリポジトリ内に残っていない（F-1 / F-2 / F-19 / F-21）
...
* **V-17 / V-18 の実測結果を可能なら記録すること。** Cryptomator 上でセルフテストが `OK` を返す可能性は互換性プローブの既知の限界であり、それだけで安全性が保証されたとは扱わない。追加可能な測定項目（例: `os.replace` 後の即時 `stat` 整合性）を検討しつつ、検出不能な性質は開発計画書 2.4 とUIへ明記する。
