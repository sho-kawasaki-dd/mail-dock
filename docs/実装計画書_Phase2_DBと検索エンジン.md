...
- [ ] `tools/bench_fts.py` を新規作成する（`tools/` は Git 管理下、生成データは管理外）
- [ ] `tests/support/eml_builder.py` を再利用してEMLを組み立てる（D-14。重複実装を作らない）
- [ ] 日本語本文で現実的な文長分布を再現する（中央値 数KB、裾に数十〜数百KBの長文を混ぜる）
- [ ] 件名・差出人（表示名＋アドレス）・添付ファイル名も生成し、`message_contents` の4列すべてに投入する
- [ ] 投入は `normalize_for_search()` を通す（**本番と同一経路であることを保証する**）
- [ ] **1千 / 5千 / 1万通**の3水準を生成し、線形性を確認できるようにする
- [ ] 生成物（DB・EML）を `.gitignore` へ追加する

#### **A-2. 実行環境の前提確認**

- [ ] `sqlite3.sqlite_version` が trigram トークナイザ対応（**3.34.0 以上**）であることを確認する
- [ ] **Windows と Linux(WSL) の双方**で確認し、バージョン差を記録する
- [ ] 未対応環境が存在する場合の扱い（起動時チェックの要否）を判断し、結論を A-5 に記録する
