...
- [x] `test_search_flow.py`: 実ストレージルート + 実SQLite で同期 → 検索の end-to-end
  - [x] 日本語本文がヒットすること
  - [x] **ひらがなとカタカナを同一視しない**こと（開発計画書 4.5 の明示要件）
  - [x] 全角英数で入力しても半角の本文にヒットすること（NFKC）
  - [x] 大文字小文字を区別しないこと（casefold）
  - [x] CP932 機種依存文字を含む本文がヒットすること
  - [x] 添付ファイル名で検索できること、**インライン画像の名前ではヒットしない**こと
  - [x] 2文字キーワードが LIKE 経路で発見できること
- [x] **正規化一貫性の回帰テスト**: 投入側（`SqliteMessageRepository._normalized_contents`）と検索側（`parse_query`）が同一関数を経由していることを、片方だけを差し替えると落ちる形で固定する（F-3）

#### **F-3. ベンチマーク（手動実行）**

- [x] `tools/bench_fts.py` を単体で実行でき、A-3 の全項目を再計測できること
- [x] 通常の `pytest` 実行に含まれないこと（D-5）
- [x] 実行方法と計測結果の読み方を `README.md` に追記する

#### **F-4. CI**

- [x] 新規テストが `lint` / `test-windows` / `test-linux` の3ジョブで緑になることを確認する
- [x] ベンチマークが CI で実行されないことを確認する
