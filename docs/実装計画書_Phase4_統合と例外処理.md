...
- [x] 「ファイル」メニューに「mbox としてエクスポート」「添付ファイルを一括抽出」を追加する
- [x] 対象は「選択したメール」または「現在の一覧（フィルタ・検索結果）全体」から選べるようにする
- [x] エクスポートを `SyncWorker`（ファイル書き込み系の既存経路）で実行し、進捗表示とキャンセルを設ける
...
- [x] `config.sync_interval_minutes`（既定60、0で無効）の `QTimer` を `MainWindow` に実装する
- [x] 前回の同期が実行中ならスキップする
- [x] ストレージが `ATTACHED` 以外のとき、**ダイアログを出さず静かにスキップ**する
- [x] `QSystemTrayIcon` を実装し、同期状態（待機中／同期中／エラー／切断）をアイコンとツールチップで表示する
- [x] トレイメニュー: 「開く」「今すぐ同期」「終了」
- [x] **ウィンドウを閉じるとトレイへ最小化**し、終了はトレイメニューまたはファイルメニューからのみ可能にする（D-15）
- [x] 最小化中も同期・定期同期が継続すること
- [x] トレイが利用できない環境（`QSystemTrayIcon.isSystemTrayAvailable()` が偽）では、閉じる＝終了へフォールバックする

#### **G-2. `sync_failures` の運用UI（F-38 / F-39）**

- [x] 「要確認」一覧（`attempt_count >= 10`）を表示するダイアログを追加する
- [x] `oversize` のメッセージに対する「それでも取得する」を実装し、サイズ上限を無視して単一メールを取得する経路を `sync_mail.py` へ追加する
- [x] `parse` 失敗のメッセージに対する再解析をGUIから実行できるようにする（既存 `reparse.py` を利用）
- [x] 一覧に失敗種別（`transient` / `permanent` / `parse` / `oversize`）と試行回数・最終失敗日時を表示する

#### **G-3. 監査ログ表示画面（F-40）**

- [x] 「ツール」メニューに「監査ログ」を追加する
- [x] `audit_log` を `occurred_at DESC` でページング表示する（読み取り専用）
- [x] 表示項目: 日時・操作・アカウント・Message-ID・件名・サイズ・詳細
- [x] **削除・編集の導線を作らない**（D-22）
- [x] 件名・メールアドレスは既存のマスキング規約に従って表示する

#### **G-4. DBバックアップとログ保持（F-41 / F-42）**

- [x] `sqlite3.Connection.backup()` により **週1回および終了時**に `metadata.db.bak` を作成する
- [x] 最終バックアップ日時を `app_state` に保持し、週1回の判定に使う
- [x] `config.db_backup_to_local_disk` を実処理へ接続する（既定OFF）
- [x] 有効化時、複製先の暗号化状態が保管元より弱い場合は警告し、既定では実行しない（Phase 3.5 D-16）
- [x] `{storage_root}/logs/sync-*.log` を `config.sync_log_retention_days`（既定90日）を過ぎたら削除する
- [x] **`audit_log` テーブルは削除対象にしない**ことをコメントとテストで固定する
...
- [x] `tests/support/fault_injection.py` を作成し、`BaseEmlStorage` とDB接続をラップして N回目の操作で `OSError(winerror=1167)` / `sqlite3.OperationalError(SQLITE_IOERR)` を注入できるようにする
- [x] `tests/unit/test_detach_faults.py` に、以下4点それぞれで切断を注入するテストを書く
    - [x] ① EML の fsync 前
    - [x] ② `os.replace` の直前
    - [x] ③ マニフェスト追記の**行途中**（部分書き込みを再現する）
    - [x] ④ DBコミット中
- [x] 各注入点で、**不変条件2で許容される状態（DB未登録のEML、またはマニフェスト済みでDB未登録の項目）にしかならない**ことを検証する
- [x] 「DBにあるが実体が無い」状態が**一度も発生しない**ことを検証する
- [x] DBコミット失敗時に checkpoint が追記されないこと、およびそのバッチが次回の範囲限定検証対象になることを検証する（レビュー修正案 3.1）
- [x] ③ の後に `verify_manifest()` を実行すると末尾の不完全レコードだけが切り離され、それ以前のレコードが失われないことを検証する
- [x] 各注入点からの復帰後、次回同期で欠落分が再取得されること（冪等性）を検証する
...
- [x] 開発計画書 6章「Phase 4」の行に、実機テストを Phase 4 では実施せず延期した旨と、その代替（フォールト注入）を追記する
- [x] 開発計画書 5.11 の「リムーバブル時の `synchronous`」に、Phase 4 では実測できず `NORMAL` を維持した旨を追記する
- [x] `README.md` に「ストレージを安全に取り外す」手順と、ドライブポリシー「クイック取り外し」の推奨を追記する
- [x] `README.md` に `verify --mode` と `reindex` の使い方を追記する
- [x] 本書「7.」へ、延期した手動検証手順（VHDX切断シナリオ、フルスケール同期、`synchronous` 実測）を記載する
