...
- [ ] A-1: checkpoint、snapshot、purge、remote deleteイベントのスキーマを更新した
- [ ] A-3: checkpointの位置をDBコミット後へ変更した
- [ ] C-1: 検証用ポートとチャンク読み要件を追加した
- [ ] C-2: DB固有IDに依存しない再構築方式を追加した
- [ ] C-2: 新DB作成と原子的入れ替えの所有者を明確にした
- [ ] C-1: orphanの再登録条件と隔離条件を明記した
- [ ] D-1: 未完了purge intentの回復手順を追加した
- [ ] E-2: remote deleteの不確定状態と再照合を追加した
- [ ] E-2: UID EXPUNGE非対応時のexpunge拒否を追加した
- [ ] B-1/B-3: probeイベントとFOREIGN遷移を追加した
- [ ] D-11/F-31: `trash / expunge`へ用語を統一した
- [ ] D-18/F-29: 汎用CSVと削除ドライランCSVのスコープを分離した
- [ ] H-1/H-5: 破壊操作の途中停止テストを追加した
- [ ] V-2/V-3/V-4: 新しい復旧条件と意味的一致条件を反映した
